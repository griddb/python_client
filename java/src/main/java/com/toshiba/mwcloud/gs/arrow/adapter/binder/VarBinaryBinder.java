...
public class VarBinaryBinder<T extends FieldVector & ElementAddressableVector>
   ...
   * @param gsType The GSType code.
   */
  public VarBinaryBinder(T vector, GSType gsType) {
    super(vector, gsType);
  ...
  public void bind(Row row, int columnIndex, int rowIndex)
    ...
    try {
        Blob blobData = new SerialBlob(binaryData);
        row.setBlob(columnIndex, blobData);
    } catch (SQLException e) {
        throw new GSException("No column binder implemented for type Binary.", e);
    }
  }
}
