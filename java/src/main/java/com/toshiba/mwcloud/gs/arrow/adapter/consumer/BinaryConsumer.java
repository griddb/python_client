...
/**
 * Consumer which consume binary type values from {@link RowSet}. Write the data to {@link
...
public abstract class BinaryConsumer extends BaseConsumer<VarBinaryVector> {
  ...
  static class NullableBinaryConsumer extends BinaryConsumer {
    ...
    public void consume(Row row) throws GSException, IOException {
      try {
        Blob bb = row.getBlob(columnIndexInRowSet);
        if (!row.isNull(columnIndexInRowSet)) {
          consume(bb.getBinaryStream());
        }
        moveWriterPosition();
      } catch  (SQLException e) {
        throw new IOException("Failed to consume blob", e);
      }
    }
  }
